...
命令面板「发布当前笔记到公众号草稿箱」，或在文件菜单选择「发布到公众号草稿箱」。弹窗里可以选公众号和排版主题，修改标题、作者、摘要，查看发布前检查和排版预览，然后发到草稿箱；也可以只复制公众号格式。Agent 顶部的手机图标、命令面板和文件菜单都能打开随笔记更新的公众号预览；预览与 Agent 共用右侧栏页签。视图右上角的原生操作可切换手机与宽幅、深色模拟或打开草稿弹窗。手机框使用固定机身宽高比、屏幕内滚动，不打包 Apple 官方设备素材。
