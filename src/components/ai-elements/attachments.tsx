...
export function Attachments({ files, onRemove, variant = "inline" }: { files: ChatAttachment[]; onRemove?: (id: string) => void; variant?: "inline" | "grid" }) {
  if (!files.length) return null;
  return <div className={`qa-attachments qa-attachments--${variant}`}>{files.map((file) => <div className="qa-attachment" key={file.id}>
    {file.mediaType.startsWith("image/") && file.url ? <img src={file.url} alt={file.name} width={variant === "grid" ? undefined : 36} height={variant === "grid" ? undefined : 36} /> : <FileText size={16} aria-hidden="true" />}
