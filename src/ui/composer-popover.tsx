...
export function ComposerPopover({ label, trigger, disabled, className = "", children }: {
  ...
  return <div ref={root} className={`qa-control ${className}`} onBlur={(event) => {
    ...
    <button ref={button} type="button" className="qa-control-trigger" aria-labelledby={`${id}-trigger`} aria-expanded={open}
      aria-haspopup="dialog" aria-controls={open ? id : undefined} disabled={disabled} onClick={() => setOpen(!open)}>{trigger}<span id={`${id}-trigger`} className="qiaomu-agent__sr-only">{label}</span></button>
...
export function effortLabel(value: string) {
  return ({ "": "默认", low: "低", medium: "中", high: "高", xhigh: "极高", minimal: "最小", none: "无", max: "最高", ultra: "超高" } as Record<string, string>)[value] ?? value;
}
